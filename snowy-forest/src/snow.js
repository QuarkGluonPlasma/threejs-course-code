...
// 使用纹理加载器加载指定路径的雪花纹理图片
const texture = loader.load("../public/snow.png");
// 创建一个精灵材质对象，将加载的雪花纹理应用到材质上
const spriteMaterial = new THREE.SpriteMaterial({
    map: texture
});

// 创建一个 Three.js 组对象，用于将多个雪花精灵对象组合在一起
const group = new THREE.Group();

// 使用 for 循环创建 1000 个雪花精灵对象
for (let i = 0; i < 1000; i ++) {
    // 创建一个雪花精灵对象，使用之前创建的精灵材质
    const sprite = new THREE.Sprite(spriteMaterial);
    // 设置雪花精灵对象在 X、Y 轴上的缩放比例为 5，Z 轴保持默认
    sprite.scale.set(15, 15);

    // 生成随机的 X 坐标，范围在 -1500 到 1500 之间
    const x = -1500 + 3000 * Math.random();
    // 生成随机的 Y 坐标，范围在 0 到 1000 之间
    const y = 1000 * Math.random();
    // 生成随机的 Z 坐标，范围在 -1500 到 1500 之间
...
function render() {
    ...
    group.children.forEach(sprite => {
        // 让雪花精灵对象沿 Y 轴负方向移动，移动速度由时间差和系数 30 决定
